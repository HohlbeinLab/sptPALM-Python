...
def load_localisations_from_csv(para):
    ...
    for index, row in df.iterrows():
        ...
        if row['internal_naming'] == 'movie_id': # Fill 'movie_id' column (preset to 1)
        ...
        elif row['internal_naming'] == 'cell_id' and row['external_naming'] is False: # Fill 'cells_id' column (preset to -1)
            csv_data['cell_id'] = -1
        elif row['internal_naming'] == 'track_id' and row['external_naming'] is False:  # Fill 'track_id' column (preset to -1)
            csv_data['track_id'] = -1
            ...
            if row['external_naming'] in csv_input_file.columns:
                ...
                print(f" ATTENTION, '{row['internal_naming']}' not found in {para['fn_locs']}!")
                print(f" ATTENTION, '{row['internal_naming']}' rows are set to -1!")
