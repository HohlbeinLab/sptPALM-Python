...
FitOption = "MLE" // use "Phasor" or "MLE"
...
//JH 2022-10-26: threshold 20 seems to be okay when MedianFilter is off, otherwise use 10 or 12 (now: std(Wave.F1)*1.2)
	...
	run("Camera setup", "readoutnoise=0.0 offset=414.0 quantumefficiency=1.0 isemgain=false photons2adu=3.6 pixelsize=119.0");
...
//// close table movie when more than filee was selected
	if(files.length >1){
		//closes all windows
		close("*");
		//non of the following things )or combinations thereof seem to work. No idea why.
		//run("Clear Results");
