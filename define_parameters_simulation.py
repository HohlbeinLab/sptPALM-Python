...
def define_parameters_simulation():
    ...
    if sim_input['number_species'] > len(sim_input['species']):
    ...
    for ii in range(sim_input['number_species']):
        species = sim_input['species'][ii]
        if len(species['diff_Quot']) < species['number_states']:
