...
def fit_data_with_MCDDA_sptPALM(D_track_length_matrix, sim_input):
    ...
    shiftX = start_values_fitting

    # Options for curve fitting (using scipy's least_squares)
    if sim_input['perform_fitting']:
        breakpoint()
        res = least_squares(fitFunc, start_values_fitting, bounds=(lower_bounds, upper_bounds), args=(sim_input,))
        shiftX = res.x
    ...
    for i, ax in enumerate(axs.flat):
        ...
        ax.step(edges, out_initial_guess.loc[out_initial_guess.index[:],sim_input['track_lengths'][i]], color = 'red')  # 'count' corresponds to `density=False`
  
        # Plot final fit results if fitting was performed
        if sim_input['perform_fitting']:
            # ax.plot(out_final_fit[:, 0], out_final_fit[:, i + 1], color='black')
            ax.step(edges, out_final_fit.loc[out_final_fit.index[:],sim_input['track_lengths'][i]], acolor = 'black')  # 'count' corresponds to `density=False`
        ...
        partA = out_initial_guess.loc[out_initial_guess.index[:-1],sim_input['track_lengths'][i]]       
        partB = D_track_length_matrix_normalized.loc[:-1, sim_input['track_lengths'][i]]
        print(f"ii: {i}, sum-of-squares: {np.sum(( partA - partB )**2)}")
...
def fitFunc(start_values_fitting, sim_input):
    ...
    if sim_input['species'][ii]['#_states'] == 2:
        ...
        sim_input['species'][ii]['diff_quot'] = start_values_fitting[0:2] / sim_input['multiplicator']
    ...
    # sim_input.species(ii).rates = [0,kAB,kAC; kBA,0,kBC; kCA,kCB,0];
        ...
        sim_input['species'][ii]['diff_quot'] = start_values_fitting[0:3] / sim_input['multiplicator']
